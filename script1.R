...
df2 <- lapply(df1$V4 %>% unique,function(x) {
  x1 <- df1 %>% 
    filter(V4==x)
  x1 %>% 
    mutate(stay_at_home = Freq/sum(x1$Freq))
}) %>% 
  do.call(rbind,.) %>% 
  apply(2,as.numeric) %>% 
  as.data.frame()
...
df3 <- data.frame(country = c("Austria","Denmark","Germany","Sweden","United Kingdom","USA"),
           ...
           predicted_penalty = c(.51,.21,.61,.27,.44,.31))


df4 <- left_join(df3,df2,by=c("code" = "V4")) %>% 
  filter(V12==3) 

ct <- lm(predicted_penalty ~ stay_at_home,data = df4)
df4 %>% 
